  ...
  return (
    <main>
      ...
      {/* ═══════════════════════════════════════════════════════
          ...
          ═══════════════════════════════════════════════════════ */}
      <section className="party-full-menu">
        <div className="container">
          <div className="party-section-header">
            <span className="party-section-tag">Full Menu</span>
            <h2 className="party-section-title">Everything on Our Menu</h2>
            <p className="party-section-desc">
              Browse our full selection &mdash; from silogs and pasta to halo-halo
              and snacks. Every item can be added to your party order.
            </p>
          </div>

          {menuCategories.map((category) => (
              <div key={category.id} className="party-menu-category">
                <h3 className="party-menu-cat-title">
                  {category.label}
                  {category.priceNote && (
                    <span className="party-menu-cat-note">{category.priceNote}</span>
                  )}
                  {category.note && (
                    <span className="party-menu-cat-note">{category.note}</span>
                  )}
                </h3>
                <div className="party-menu-grid">
                  {category.items.map((item) => {
                    const imgSrc = item.image || category.categoryImage;
                    const itemPrice = item.price != null
                      ...
                      : item.variants
                        ...
                        : category.priceNote || '';
                    return (
                      <div key={item.id} className="party-menu-item">
                        <div className="party-menu-item-img-wrap">
                          {imgSrc ? (
                            <img
                              src={imgSrc}
                              alt={item.name}
                              className="party-menu-item-img"
                              loading="lazy"
                            />
                          ) : (
                            <div className="party-menu-item-img-placeholder">
                              <span className="material-symbols-outlined">restaurant_menu</span>
                            </div>
                          )}
                        </div>
                        <div className="party-menu-item-body">
                          <div className="party-menu-item-info">
                            <span className="party-menu-item-name">{item.name}</span>
                            <span className="party-menu-item-price">{itemPrice}</span>
                          </div>
                          {item.badge && (
                            <span className="party-menu-item-badge">{item.badge}</span>
                          )}
                          <p className="party-menu-item-desc">{item.description}</p>
                          <span className="party-menu-item-meta">{item.serves} &middot; {item.prepTime}</span>
                        </div>
                      </div>
                    );
                  })}
      ...
      <style>{`
        ...
        .party-menu-category + .party-menu-category {
          margin-top: 2.5rem;
          padding-top: 2.5rem;
          border-top: 1px solid var(--color-outline-variant);
        }

        .party-menu-cat-title {
          font-family: var(--font-headline);
          font-size: 1.125rem;
          font-weight: 700;
          color: var(--color-primary);
        ...
        .party-menu-item:hover .party-menu-item-img {
          transform: scale(1.06);
        }
        .party-menu-item-img-placeholder {
          width: 100%;
          height: 100%;
          display: flex;
          align-items: center;
        ...
        .party-menu-item-img-placeholder .material-symbols-outlined {
          font-size: 32px;
        }

        .party-menu-item-body {
        ...
        .party-menu-item-info {
          display: flex;
          align-items: baseline;
          justify-content: space-between;
          gap: 0.75rem;
        }
        .party-menu-item-name {
          font-size: 0.875rem;
          font-weight: 700;
          color: var(--color-on-surface);
          line-height: 1.2;
        }
        .party-menu-item-price {
          flex-shrink: 0;
          font-size: 0.875rem;
          font-weight: 800;
          color: var(--color-primary);
        }
        .party-menu-item-badge {
          align-self: flex-start;
          font-size: 0.625rem;
          font-weight: 700;
          text-transform: uppercase;
